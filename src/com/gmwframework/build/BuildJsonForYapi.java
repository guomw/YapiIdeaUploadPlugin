package com.gmwframework.build;
...
import com.gmwframework.constant.*;
import com.gmwframework.dto.YapiApiDTO;
import com.gmwframework.dto.YapiHeaderDTO;
import com.gmwframework.dto.YapiPathVariableDTO;
import com.gmwframework.dto.YapiQueryDTO;
import com.gmwframework.upload.UploadYapi;
import com.gmwframework.util.DesUtil;
import com.gmwframework.util.FileToZipUtil;
import com.gmwframework.util.FileUnZipUtil;
import com.gmwframework.util.PsiAnnotationSearchUtil;
...
public class BuildJsonForYapi {
    ...
    public ArrayList<PsiJavaFile> getPsiJavaFiles(PsiElement[] psiElements) {
        ArrayList<PsiJavaFile> arrayList = new ArrayList<>();
        if (psiElements == null) {
            return arrayList;
        }
        for (PsiElement element : psiElements) {
            if (element instanceof PsiDirectory) {
                arrayList.addAll(getPsiJavaFiles(element.getChildren()));
            }
    ...
    public ArrayList<YapiApiDTO> actionPerformedList(Project project, PsiJavaFile psiJavaFile, String attachUpload, String returnClass) {
        String selectedText = null;
        PsiClass selectedClass = psiJavaFile.getClasses()[0];
        PsiFile psiFile = psiJavaFile.getOriginalFile();
    ...
    public static YapiApiDTO actionPerformed(PsiClass selectedClass, PsiMethod psiMethodTarget, Project project, PsiFile psiFile, String attachUpload, String returnClass) {
        ...
        if (psiAnnotationMethod != null) {
            ...
            if (psiAnnotationMethodSemple != null) {
                ...
                if (psiNameValuePairs != null && psiNameValuePairs.length > 0) {
                    for (PsiNameValuePair psiNameValuePair : psiNameValuePairs) {
                        //获得方法上的路径
                        if (Objects.isNull(psiNameValuePair.getName()) || psiNameValuePair.getName().equals("value") || psiNameValuePair.getName().equals("path")) {
                            PsiReference psiReference = psiNameValuePair.getValue().getReference();
                            if (psiReference == null) {
                                DesUtil.addPath(path, psiNameValuePair.getLiteralValue());
                            } else {
        ...
        // 2019-12-05 郭孟稳添加 获取swagger Api注解的tags值当分组菜单
        String tags = PsiAnnotationSearchUtil.findAnnotationValueByAttributeKey(selectedClass, SwaggerConstant.ApiTags, YapiAttributeKeyConstant.tags);
        if (Objects.nonNull(tags)) {
            yapiApiDTO.setMenu(tags);
        }

        // 2019-12-05 郭孟稳添加 获取swagger ApiOperation注解的name值当接口名称
        String title = PsiAnnotationSearchUtil.findAnnotationValueByAttributeKey(psiMethodTarget, SwaggerConstant.ApiOperation, YapiAttributeKeyConstant.value);
    ...
    public static List<Map<String, String>> getRequestForm(Project project, PsiParameter psiParameter, PsiMethod psiMethodTarget) {
        ...
        if (NormalTypes.normalTypes.containsKey(psiParameter.getType().getPresentableText())) {
            ...
            for (PsiField field : psiClass.getAllFields()) {
                ...
                String remark = PsiAnnotationSearchUtil.findAnnotationValueByAttributeKey(field, SwaggerConstant.ApiModelProperty, YapiAttributeKeyConstant.value);
                if (field.getDocComment() != null && Strings.isNullOrEmpty(remark)) {
...
//                remark = DesUtil.getLinkRemark(remark, project, field);
                ...
                //如果是有notnull 和 notEmpty 或者ApiModelProperty 注解required==true就加入必填
                if (Objects.nonNull(PsiAnnotationSearchUtil.findAnnotation(field, JavaConstant.NotNull))
                        ...
                        || checkApiModelPropertyRequired(field)
                ){
                    map.put("required","1");
                }
                else{
...
//                getFilePath(project, filePaths, DesUtil.getFieldLinks(project, field));
            ...
            if (Objects.nonNull(psiClass.getSuperClass()) && Objects.nonNull(NormalTypes.collectTypes.get(psiClass.getSuperClass().getName()))) {
                for (PsiField field : psiClass.getFields()) {
                    if (Objects.nonNull(PsiAnnotationSearchUtil.findAnnotation(field, JavaConstant.Deprecate))
                            || checkApiModelPropertyHidden(field)
                    ) {
                        continue;
                    }
    ...
    /**
     * <p>检查字段是否必填</p>
     * 功能描述: 读取swagger ApiModelProperty 注解 required 属性
     *
     * @param field
     * @return boolean
     * @author guomw
     ...
     * @since V1.0.0
     */
    private static boolean checkApiModelPropertyRequired(PsiField field) {
        try {
            String required = PsiAnnotationSearchUtil.findAnnotationValueByAttributeKey(field, SwaggerConstant.ApiModelProperty, YapiAttributeKeyConstant.required);
    ...
    /**
     * <p>检查字段是否隐藏</p>
     * 功能描述:读取swagger ApiModelProperty 注解 hidden 属性
     *
     * @param field
     * @return boolean
     * @author guomw
     * @date 11:42 2019/12/5
     * @since V1.0.0
     */
    private static boolean checkApiModelPropertyHidden(PsiField field) {
        try {
            String hidden = PsiAnnotationSearchUtil.findAnnotationValueByAttributeKey(field, SwaggerConstant.ApiModelProperty, YapiAttributeKeyConstant.hidden);
    ...
    public static void getField(PsiField field, Project project, KV kv, String[] childType, Integer index, Set<String> pNames) {
        ...
        if (type instanceof PsiPrimitiveType) {
            ...
            // 判断字段是否包含ApiModelProperty 注解,定义属性example为mock规则内容
            JsonObject mock = PsiAnnotationSearchUtil.findAnnotationMockValue(field, SwaggerConstant.ApiModelProperty, YapiAttributeKeyConstant.example);
            if (Objects.nonNull(mock)) {
                jsonObject.add("mock", mock);
            } else {
